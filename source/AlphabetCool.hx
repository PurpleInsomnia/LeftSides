 ...
 */
class AlphabetCool extends FlxSpriteGroup
{
	...
	var lastSprite:AlphaCharacterCool;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public static var isBold:Bool = false;
	public var lettersArray:Array<AlphaCharacterCool> = [];
	...
	public static var textSound:String = 'warnType';
	...
	*/
	...
	public function new(x:Float, y:Float, text:Dynamic = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = 0.05, ?textSize:Float = 1)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = AlphaCharacterCool.numbers.indexOf(character) != -1;
			var isSymbol:Bool = AlphaCharacterCool.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = AlphaCharacterCool.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterCool = new AlphaCharacterCool(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterCool = new AlphaCharacterCool(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = AlphaCharacterCool.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = AlphaCharacterCool.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = AlphaCharacterCool.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterCool = new AlphaCharacterCool(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterCool = new AlphaCharacterCool(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class AlphaCharacterCool extends FlxSprite
{
	...
	{
		super(x, y);
		var tex = Paths.getSparrowAtlas('alphabetButCool');
		if (AlphabetCool.isBold)
		{
			tex = Paths.getSparrowAtlas('alphabetButCool');
		}
		else
		{
			tex = Paths.getSparrowAtlas('alphabetWhite');
		}
		frames = tex;
