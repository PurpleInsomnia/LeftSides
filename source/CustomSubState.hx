...
class CustomSubState extends MusicBeatSubstate
{
	...
	var scriptArray:Array<SStateScript> = [];
    ...
    public function new(name:String)
    {
        ...
        SStateHscript.initialize();
        setHscriptVars();

        scriptArray.push(SStateHscript.load(Paths.getModFile("states/substates/" + name + ".hxs"), hscriptVars));
    }
	...
	{
		// goofyahh
		var daScript:SStateScript = scriptArray[ind];
		daScript.dispose();
...
class SStateHscript
{
	...
	public static function initialize()
	{
		...
		exp.set("FlxTypedGroup", SStateGroup);
		...
		exp.set("FlxDirectionFlags", SStateDirection);
		exp.set("FlxCollision", FlxCollision);
		exp.set("FlxFlicker", FlxFlicker);
		exp.set("FlxTweenType", SStateType);
        exp.set("FlxTextBorderStyle", SStateBorder);
	...
	{
		return new SStateScript(parser.parseString(File.getContent(path)), extraParams);
	}
}
...
class SStateScript
{
	...
	{
		interp = new Interp();
		for (i in SStateHscript.exp.keys())
			interp.variables.set(i, SStateHscript.exp.get(i));
		if (extraParams != null)
		{
...
class SStateGroup extends FlxTypedGroup<Dynamic>
{
    public function new()
    {
        super();
