...
// basically the MODS menu from later versions of psych engine lmfao
class DlcMenuState extends MusicBeatState
{
	...
	public static var modsList:Array<Dynamic> = [];
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In the DLC menu", null);
		#end
		...
		noDlcText = new FlxText(0, 0, FlxG.width, 'NO DLC FOUND\nPlease Install A Dlc Pack By Placing\nIts folder into the "Mods" folder!', 48);
		noDlcText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		noDlcText.scrollFactor.set();
		noDlcText.borderSize = 2;
		add(noDlcText);
		noDlcText.screenCenter();
		visibleWhenNoMods.push(noDlcText);
		...
		if(FileSystem.exists(path))
		{
			...
			for (i in 0...leMods.length)
			{
				if(leMods.length > 1 && leMods[0].length > 0) {
					...
					if(!Paths.ignoreModFolders.exists(modSplit[0].toLowerCase()))
					{
						if (modSplit[1] == '1')
							fileArray.push(modSplit[0]);
						//trace(modSplit[1]);
					}
				}
			}
		...
		if (FileSystem.exists("modsList.txt")){
			for (folder in Paths.getModDirectories())
			{
				if(!Paths.ignoreModFolders.exists(folder))
				{
					...
					{
						if (fileArray.contains(Std.string(folder)))
		...
		//attached buttons
		var startX:Int = Std.int(selector.x + 780);

		buttonToggle = new FlxButton(startX, 0, "TURN OFF", function()
		{
		...
		installButton = new FlxButton(startX, 620, "Open .Exe File", function()
		{
			openExe();
		});
		...
		removeButton = new FlxButton(startX, 620, "Remove Selected DLC", function()
		{
			...
			if(FileSystem.exists(path) && FileSystem.isDirectory(path))
			{
				...
				try
				{
					...
					// Not gonna put this in Incase you like.... press this when you didn't intend too.
		...
		///////
		descriptionTxt = new FlxText(selector.x + 20, 0, (Std.int(selector.x + 850)) - 216, "", 32);
		descriptionTxt.setFormat(Paths.font("eras.ttf"), 32, FlxColor.WHITE, LEFT);
		...
		while (i < modsList.length)
		{
			...
			newMod.alphabet.screenCenter(X);
			newMod.alphabet.y = i * 150;
			newMod.alphabet.x = selector.x;
			//Don't ever cache the icons, it's a waste of loaded memory
			var loadedIcon:BitmapData = null;
			var iconToUse:String = Paths.mods(values[0] + '/bg.png');
			if(FileSystem.exists(iconToUse))
			{
				loadedIcon = BitmapData.fromFile(iconToUse);
			}

			newMod.icon = new FlxSprite();
			if(loadedIcon != null)
			{
				newMod.icon.loadGraphic(loadedIcon, true, 850, 450);//animated icon support
				var totalFrames = Math.floor(loadedIcon.width / 850) * Math.floor(loadedIcon.height / 450);
				newMod.icon.animation.add("icon", [for (i in 0...totalFrames) i],10);
				newMod.icon.animation.play("icon");
			}
			else
			{
				newMod.icon.loadGraphic(Paths.image('placeholderBg'));
			}
			newMod.icon.screenCenter();
			add(newMod.alphabet);
			add(newMod.icon);
			i++;
		}
	...
	function updateButtonToggle()
	{
		...
		{
			buttonToggle.label.text = 'TURN OFF';
			buttonToggle.color = FlxColor.RED;
		}
		else
		{
			buttonToggle.label.text = 'TURN ON';
	...
	{
		if(noDlcText.visible)
		{
			noModsSine += 180 * elapsed;
			noDlcText.alpha = 1 - Math.sin((Math.PI * noModsSine) / 180);
		}
		...
		if(canExit && controls.BACK)
		{
			...
			else
			{
				...
				{
					FlxG.sound.music.fadeOut(0.3);
					MusicBeatState.switchState(new DlcLoader());
				}
				else
				{
	...
	{
		selector.makeGraphic(850, 450, FlxColor.TRANSPARENT);
		selector.screenCenter();
	}

	var _file:FileReference = null;
	function openExe() {
		var exeFilter:FileFilter = new FileFilter('EXE', 'exe');
		_file = new FileReference();
		_file.addEventListener(Event.SELECT, onLoadComplete);
		_file.addEventListener(Event.CANCEL, onLoadCancel);
		_file.addEventListener(IOErrorEvent.IO_ERROR, onLoadError);
		_file.browse([exeFilter]);
		canExit = false;
	}
	...
	function onLoadComplete(_):Void
	{
		...
		if(fullPath != null)
		{
			var rawExe:String = File.getContent(fullPath);
			if(rawExe != null)
			{
				var coolExe:FileReference = new FileReference();
				coolExe.load();
			}
		}
		// _file = null;
...
class ModMetadata
{
	...
	public function new(folder:String)
	{
		...
		this.color = DlcMenuState.defaultColor;
		this.restart = false;
