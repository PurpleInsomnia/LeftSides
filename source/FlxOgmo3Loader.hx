...
class FlxOgmo3Loader
{
	 ...
	 */
	public function loadDecals(decalLayer:String = 'decals', decalsPath:String):FlxTypedGroup<FlxSprite>
	{
		if (!decalsPath.endsWith('/'))
			decalsPath += '/';
		var g = new FlxTypedGroup<FlxSprite>();
		for (decal in level.getDecalLayer(decalLayer).decals)
		{
			var s = new FlxSprite(decal.x, decal.y);
			s.loadGraphic(decalsPath + decal.texture);
