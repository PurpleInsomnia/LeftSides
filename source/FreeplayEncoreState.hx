...
class FreeplayEncoreState extends MusicBeatState
{
	var songs:Array<EncoreSongMetadata> = [];
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Freeplay Menu (ENCORE)", null);
		#end
	...
	{
		songs.push(new EncoreSongMetadata(songName, weekNum, songCharacter, color));
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (accepted)
		{
			...
			#if MODS_ALLOWED
			if(!sys.FileSystem.exists(Paths.modsJson(songLowercase + '/' + poop)) && !sys.FileSystem.exists(Paths.json(songLowercase + '/' + poop + '-encore'))) {
			#else
			if(!OpenFlAssets.exists(Paths.json(songLowercase + '/' + poop + '-encore'))) {
			#end
				poop = songLowercase;
				curDifficulty = 1;
			...
			PlayState.SONG = Song.loadFromJson(poop + '-encore', songLowercase);
			...
			{
				LoadingState.loadAndSwitchState(new EncoreChartingState());
			}
			else
			{
		...
		{
			openSubState(new ResetEncoreScoreSubState(songs[curSelected].songName, curDifficulty, songs[curSelected].songCharacter));
	...
	function changeDiff(change:Int = 0)
	{
		...
		#if !switch
		intendedScore = Highscore.getEncoreScore(songs[curSelected].songName, curDifficulty);
		intendedRating = Highscore.getEncoreRating(songs[curSelected].songName, curDifficulty);
		intendedRank = Highscore.getEncoreSongRank(songs[curSelected].songName, curDifficulty);
		#end
