...
class IntroState extends MusicBeatState
{
	...
	function onComplete(Tween:FlxTween):Void
	{
		...
		FlxG.switchState(new PurpleInsomniaSplashState());
	}
}

class PurpleInsomniaSplashState extends MusicBeatState
{
	var introSound:FlxSound;
	var meBG:FlxSprite;
	var me:FlxSprite;
	var hidden:FlxSprite;
	override function create()
	{
		meBG = new FlxSprite().loadGraphic(Paths.image("purpleinsomniaBG"));
		meBG.alpha = 0;
		add(meBG);

		me = new FlxSprite().loadGraphic(Paths.image('purpleinsomnia'));
		me.alpha = 0;
		add(me);
		introSound = FlxG.sound.load(Paths.sound('purpleinsomniaJingle'), 2);
		introSound.play();

		hidden = new FlxSprite().loadGraphic(Paths.image("purpleinsomniahidden"));
		hidden.visible = false;
		add(hidden);

		FlxTween.tween(me, {alpha: 1}, 3);
		FlxTween.tween(meBG, {alpha: 1}, 4);

		new FlxTimer().start(5.14, function(tmr:FlxTimer)
		{
			skip();
		});
		super.create();
	...
	{
		if (FlxG.keys.anyJustPressed([ENTER, SPACE]) && !skipping)
		{
			skipping = true;
			skip();
		}
