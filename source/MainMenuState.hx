...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (!FileSystem.exists(Paths.image('bgSprites/' + ClientPrefs.bgSprite)))
		{
			trace('oh no! our ' + ClientPrefs.bgSprite + '!  its broken!');
		}

		var yScroll:Float = Math.max(0.25 - (0.05 * (optionShit.length - 4)), 0.1);
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('bgSprites/' + ClientPrefs.bgSprite));
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.image('bgSprites/' + ClientPrefs.bgSprite + 'Desat'));
		...
		var menuInfo:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuInfo'));
		menuInfo.screenCenter();
		menuInfo.scrollFactor.set();
		add(menuInfo);

		var spikes = new FlxSprite().loadGraphic(Paths.image('menuSpikes'));
		...
		for (i in 0...optionShit.length)
		{
			...
			var menuItem:FlxSprite = new FlxSprite((i * 50), (i * 140)  + offset);
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 64, 0, "Friday Night Funkin Left Sides v1.0", 12);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			{
				selectedSomethin = true;
				MusicBeatState.switchState(new WhatsNewState());
			}
			#if desktop
