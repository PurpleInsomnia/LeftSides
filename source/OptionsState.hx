...
{
	var options:Array<String> = ['Notes', 'Controls', 'Preferences', 'Combo And Sound Offsets', 'Customize'];
	...
	override function create() {
		...
		menuBG.shader = swagShader.shader;
...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Jumpscares':
						ClientPrefs.jumpscares = !ClientPrefs.jumpscares;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Jumpscares':
				daText = "If you uncheck this, you will not see or hear SOME of the scary stuff";
			case 'Note Splashes':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Jumpscares':
						daValue = ClientPrefs.jumpscares;
					case 'Swearing':
