...
class SpookyTitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl.frames = Paths.getSparrowAtlas('logoBumpin-alt');
		...
		if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			...
			gfDance.frames = Paths.getSparrowAtlas('gfDanceTitle-alt');
	...
	{
		var fullText:String = Assets.getText(Paths.txt('introText-scary'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxG.sound.play(Paths.sound('confirmMenu-laugh'), 0.7);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['PurpleIsnomnia'], 45);
