...
class TitleLua {
	...
	var lePlayState:TitleState = null;
	var scriptName:String = '';
	var gonnaClose:Bool = false;

	private var luaArray:Array<TitleLua> = [];
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "setObjectOrder", function(obj:String, position:Int) {
			if(lePlayState.modchartSprites.exists(obj)) {
				var spr:ModchartSpriteTitle = lePlayState.modchartSprites.get(obj);
				if(spr.wasAdded) {
		...
		Lua_helper.add_callback(lua, "keyJustPressed", function(name:String) {
			var key:Bool = false;
			switch(name) {
				case 'left': key = lePlayState.getControl('UI_LEFT_P');
				case 'down': key = lePlayState.getControl('UI_DOWN_P');
				case 'up': key = lePlayState.getControl('UI_UP_P');
				case 'right': key = lePlayState.getControl('UI_RIGHT_P');
		...
		Lua_helper.add_callback(lua, "getMouseX", function() {
			return FlxG.mouse.getScreenPosition().x;
		});
		Lua_helper.add_callback(lua, "getMouseY", function() {
			return FlxG.mouse.getScreenPosition().y;
		});

		Lua_helper.add_callback(lua, "makeLuaSprite", function(tag:String, image:String, x:Float, y:Float) {
			tag = tag.replace('.', '');
			resetSpriteTag(tag);
			var leSprite:ModchartSpriteTitle = new ModchartSpriteTitle(x, y);
		...
		Lua_helper.add_callback(lua, "addAnimationByPrefix", function(obj:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			if(lePlayState.modchartSprites.exists(obj)) {
				var cock:ModchartSpriteTitle = lePlayState.modchartSprites.get(obj);
		...
		Lua_helper.add_callback(lua, "addAnimationByIndices", function(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(lePlayState.modchartSprites.exists(obj)) {
				var pussy:ModchartSpriteTitle = lePlayState.modchartSprites.get(obj);
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(lePlayState.modchartSprites.exists(tag)) {
				var shit:ModchartSpriteTitle = lePlayState.modchartSprites.get(tag);
				if(!shit.wasAdded) {
					if(front) 
					{
					...
					{
						lePlayState.modchartGroup.add(shit);
					}
					shit.wasAdded = true;
					trace('added ' + tag);
				}
		...
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0) {
			if(lePlayState.modchartSprites.exists(obj)) {
				var shit:ModchartSpriteTitle = lePlayState.modchartSprites.get(obj);
		...
		Lua_helper.add_callback(lua, "removeLuaSprite", function(tag:String, destroy:Bool = true) {
			...
			var pee:ModchartSpriteTitle = lePlayState.modchartSprites.get(tag);
			if(destroy) {
				pee.kill();
			}
		...
		Lua_helper.add_callback(lua, "skip", function() {
			lePlayState.skipIntro();
		});
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			...
			if(lePlayState.modchartSprites.exists(tag)) {
				var cock:ModchartSpriteTitle = lePlayState.modchartSprites.get(tag);
				cock.animation.addByPrefix(name, prefix, framerate, loop);
				if(cock.animation.curAnim == null) {
					cock.animation.play(name, true);
				}
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			...
			if(lePlayState.modchartSprites.exists(tag)) {
				...
				var pussy:ModchartSpriteTitle = lePlayState.modchartSprites.get(tag);
				pussy.animation.addByIndices(name, prefix, die, '', framerate, false);
				if(pussy.animation.curAnim == null) {
					pussy.animation.play(name, true);
				}
...
class ModchartSpriteTitle extends FlxSprite
{
	public var wasAdded:Bool = false;
	//public var isInFront:Bool = false;
}

class DebugLuaTextTitle extends FlxText
{
	private var disableTime:Float = 6;
	public var parentGroup:FlxTypedGroup<DebugLuaTextTitle>; 
	public function new(text:String, parentGroup:FlxTypedGroup<DebugLuaTextTitle>) {
