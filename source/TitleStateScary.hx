...
class TitleStateScary extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				gfDance.loadGraphic(Paths.image('titleFakeOut'));

				FlxG.sound.play(Paths.sound('titleFakeOut'), 0.7);

				FlxTween.tween(logoBl, {x: logoBl.x - (logoBl.width * 2)}, 0.025, {ease: FlxEase.quadInOut, type: PINGPONG});
				FlxTween.tween(titleText, {x: titleText.x + 1280}, 0.025, {ease: FlxEase.quadInOut, type: PINGPONG});
