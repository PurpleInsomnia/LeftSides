...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		bgThing = new FlxSprite().loadGraphic(Paths.image('editors/dialogueEditBG'));
		bgThing.screenCenter();
		add(bgThing);

		FlxG.sound.playMusic(Paths.music('breakfast'), 0.25);
	...
	function addDialogueLineUI() {
		...
		pixelCheckbox = new FlxUICheckBox(angryCheckbox.x, angryCheckbox.y + angryCheckbox.height, null, null, "Pixel Textbox", 200);
		pixelCheckbox.callback = function()
		{
			updateTextBox();
			dialogueFile.dialogue[curSelected].boxState = (pixelCheckbox.checked ? 'pixel' : 'normal');
		};
	...
	function updateTextBox() {
		...
		var isPixel:Bool = pixelCheckbox.checked;
		var anim:String = isAngry ? 'angry' : isPixel ? 'pixel' : 'normal';
	...
	function changeText(add:Int = 0) {
		...
		pixelCheckbox.checked = (curDialogue.boxState == 'pixel');
