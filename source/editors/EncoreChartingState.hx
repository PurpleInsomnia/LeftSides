...
class EncoreChartingState extends MusicBeatState
{
	...
	**/
	...
	function loadSong():Void
	{
		...
		var file:Dynamic = Paths.voicesEncore(currentSongName);
		vocals = new FlxSound();
	...
	function loadAudioBuffer() {
		...
		audioBuffers[0] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/InstEncore.ogg'))) {
			audioBuffers[0] = AudioBuffer.fromFile(Paths.modFolders('songs/' + currentSongName + '/InstEncore.ogg'));
			//trace('Custom vocals found');
		}
		else { #end
			var leVocals:Dynamic = Paths.instEncore(currentSongName);
		...
		audioBuffers[1] = null;
		#if MODS_ALLOWED
		if(FileSystem.exists(Paths.modFolders('songs/' + currentSongName + '/VoicesEncore.ogg'))) {
			audioBuffers[1] = AudioBuffer.fromFile(Paths.modFolders('songs/' + currentSongName + '/VoicesEncore.ogg'));
			//trace('Custom vocals found');
		} else { #end
			var leVocals:Dynamic = Paths.voicesEncore(currentSongName);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			...
			if(note.noteData < 0) {
				var daText:EncoreAttachedFlxText = new EncoreAttachedFlxText(0, 0, 400, 'Event: ' + note.eventName + ' (' + Math.floor(note.strumTime) + ' ms)' + '\nValue 1: ' + note.eventVal1 + '\nValue 2: ' + note.eventVal2, 12);
				...
				if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
					...
					var daText:EncoreAttachedFlxText = new EncoreAttachedFlxText(0, 0, 100, theType, 24);
	...
	private function saveLevel()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), Paths.formatToSongPath(_song.song) + "-encore.json");
		}
	}
	...
	private function saveEvents()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.addEventListener(Event.CANCEL, onSaveCancel);
			_file.addEventListener(IOErrorEvent.IO_ERROR, onSaveError);
			_file.save(data.trim(), "eventsEncore.json");
		}
	}
...
class EncoreAttachedFlxText extends FlxText
{
