...
class RGBShift
{
    public var shader:RGBShiftFilter = new RGBShiftFilter();

    public function new()
    {
        shader.aberration.value = [0.0];
        shader.effectTime.value = [0.0];
    }

    public function setAb(ab:Float)
    {
        shader.aberration.value[0] = ab;
    }

    public function updateTime(elapsed:Float)
    {
        shader.effectTime.value[0] += elapsed;
    }

    public function setTime(time:Float)
    {
        shader.effectTime.value[0] = time;
    }
}

class RGBShiftFilter extends FlxShader
{
	@:glFragmentSource('
    #pragma header
    /*
